...
const publisher_1 = __webpack_require__(782);
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            yield publisher_1.publish(name, path, tag, cfApiKey, cfId, wowiApiToken, wowiId, changelog);
            //core.debug(new Date().toTimeString());
...
const core = __importStar(__webpack_require__(2186));
const bent_1 = __importDefault(__webpack_require__(3113));
const fs_1 = __webpack_require__(5747);
const form_data_1 = __importDefault(__webpack_require__(4334));
function publish(name, path, tag, cfApiKey, cfId, wowiApiToken, wowiId, changelog) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const tocFile = fs_1.readFileSync(`${path}/${name}.toc`, { encoding: "utf8" });
        const lines = tocFile.split(/\r?\n/);
        const toc = {};
        for (const line of lines) {
            const result = line.match(/^\s*##\s*(\w+):\s*(.*?)\s*$/);
            if (result) {
                toc[result[1]] = result[2];
            }
        }
        if (version !== toc.Version) {
            core.setFailed(`Version in toc ${toc.Version} doesn't match tag ${version}`);
            return;
        }
        if (!toc.Interface) {
        ...
        const gameVersion = `${Math.floor(parsedVersion / 10000)}.${Math.floor(parsedVersion / 100) % 100}.${parsedVersion % 100}`;
        if (cfId) {
            let curseForgeClient = bent_1.default("json", "https://wow.curseforge.com");
            const versions = yield curseForgeClient("/api/game/versions", "GET", {
                "x-api-token": cfApiKey,
            });
            const curseGameVersion = versions.find((x) => x.gameVersionTypeID === 517 && x.name === gameVersion);
            if (!curseGameVersion) {
                core.setFailed(`Game version ${gameVersion} not found on Curseforge`);
                return;
            }
            ...
            curseForgeClient = bent_1.default("json", "POST", "https://wow.curseforge.com", [
                200,
                400,
            ]);
            const result = yield curseForgeClient(`/api/projects/${cfId}/upload-file`, formData.getBuffer(), Object.assign({
                "x-api-token": cfApiKey,
            }, formData.getHeaders()));
            if (result && result.id) {
        ...
        if (wowiId) {
            let client = bent_1.default("json", "https://api.wowinterface.com/");
            const versions = yield client("/addons/compatible.json", "GET", {
                "x-api-token": wowiApiToken,
            });
            const wowInterfaceGameVersion = versions.find((x) => x.interface === toc.Interface);
            if (!wowInterfaceGameVersion) {
                core.setFailed(`Game version ${toc.Interface} not found on WowInterface`);
                return;
            }
            const formData = new form_data_1.default();
            ...
            client = bent_1.default("json", "POST", "https://api.wowinterface.com/", [
                200,
                202,
                400,
            ]);
            const result = yield client(`/addons/update`, formData.getBuffer(), Object.assign({
                "x-api-token": wowiApiToken,
            }, formData.getHeaders()));
...
exports.ZipCrypto = __webpack_require__(3228);
...
const caseless = __webpack_require__(5684)
const bytes = __webpack_require__(5734)
const bent = __webpack_require__(3076)
...
const bytes = __webpack_require__(4391)
...
const fallback = __webpack_require__(3089).from
const bytes = __webpack_require__(4391)
